...
然而尽管 ISTA 结构简单、易于实现，但其收敛速度较慢，通常仅具有 $O(1/k)$ 的次线性全局收敛速率，这在处理大规模实际问题时效率较低。在本篇论文发表前，就已经有研究者在数学上证明了 ISTA 生成的序列 $\{\bm{x}_k\}$ 在某些关于算子 $\bm{A}$ 的病态条件下可能具有极慢并且任意差的渐进收敛速率\cite{bredies2008iterative}。因此，如何加速 ISTA 的收敛速度成为了一个重要的研究方向。本文正是在此背景下，针对 $l_1$ 正则化线性逆问题，提出了一种快速迭代收缩阈值算法（FISTA），旨在显著提升收敛效率，同时适用于更一般的非光滑复合优化问题。
...
\begin{enumerate}
    \item 理论层面：完成了 ISTA 算法的收敛性证明，提出快速迭代收缩阈值算法（FISTA）并完成收敛性证明。\\
        针对 ISTA 的慢收敛缺陷，本篇论文结合了 Nesterov 的加速思想\cite{nesterov1983method}与近端操作的适配性设计，构建了 FISTA 算法框架，还证明了 ISTA 的全局收敛速度为 $O(1/k)$，而 FISTA 则达到了 $O(1/k^2)$ 的全局收敛速率，显著优于 ISTA。这一改进是通过引入一个由历史迭代点精心构造的辅助更新点 $\bm{y}_k$ 实现的，该点由前两步迭代结果的线性组合生成，形式上为
        $$
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\linewidth]{figs/test_data1}
	\caption[Cameraman 图像去模糊的实验数据]{Cameraman 图像去模糊的实验数据}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{figs/test_res1}
	\caption[Cameraman 图像去模糊的实验结果]{Cameraman 图像去模糊的实验结果}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{figs/test_data2}
	\caption[Simple Test Image 图像去模糊的实验数据]{Simple Test Image 图像去模糊的实验数据}
\end{figure}
...
\begin{figure}[H]
	\centering \label{fig:test_res2}
	\includegraphics[width=0.5\linewidth]{figs/test_res2}
	\caption[Simple Test Image 图像去模糊的实验结果]{Simple Test Image 图像去模糊的实验结果}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{figs/test_res3}
	\caption[简单案例下的收敛速度]{简单案例下的收敛速度对比}
\end{figure}
...
\cite{monga2021algorithm} 中列出的表格表明，FISTA 等传统迭代算法在恢复质量和计算效率上往往不如现代深度学习方法，此类传统算法在现如今的应用范围已大不如前。
...
FISTA 可被用于恢复各种信号和图像，这暗示基于优化的神经网络也有可能具备类似的恢复能力。例如\cite{zhang2018ista}提出了一种基于 ISTA 的神经网络架构，称为 ISTA-Net，用于感知压缩等任务的端到端学习，其每一层的网络操作本质是对一次 ISTA 迭代的“网络化映射”。类似地，FISTA 也可以被“网络化”，形成 “FISTA-Net”，从而提升恢复性能。
...
\cite{dhurandhar2018explanations} 以 FISTA 为核心算法，探究模型以某种特征的缺失作为分类依据的可解释性，

深度学习中常使用到的稀疏自编码器（sparse autoencoder，SAE）等价于求解带 $l_1$ 正则化的优化问题，而 FISTA 可作为高效的求解器嵌入到 SAE 的训练过程中，从而提升训练效率和模型性能。
